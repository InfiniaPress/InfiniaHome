...
/**
 * Base class that represents a row from the 'infiniaconfiguration' table.
 *
 *
 *
...
abstract class Configuration implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\InfiniaHome\\DB\\Map\\ConfigurationTableMap';
    ...
    /**
     * Initializes internal state of InfiniaHome\DB\Base\Configuration object.
     */
     ...
     *
     * @return $this|Configuration The current object, for fluid interface
     */
     ...
     * @return $this|\InfiniaHome\DB\Configuration The current object (for fluent API support)
     */
    public function setKey($v)
    {
        ...
        if ($this->key !== $v) {
            $this->key = $v;
            $this->modifiedColumns[ConfigurationTableMap::COL_KEY] = true;
        }
    ...
    public function setValue($v)
    {
        ...
        if ($this->value !== $v) {
            $this->value = $v;
            $this->modifiedColumns[ConfigurationTableMap::COL_VALUE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : ConfigurationTableMap::translateFieldName('Key', TableMap::TYPE_PHPNAME, $indexType)];
            $this->key = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : ConfigurationTableMap::translateFieldName('Value', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 2; // 2 = ConfigurationTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\InfiniaHome\\DB\\Configuration'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ConfigurationTableMap::COL_KEY)) {
            $modifiedColumns[':p' . $index++]  = 'key';
        }
        if ($this->isColumnModified(ConfigurationTableMap::COL_VALUE)) {
            $modifiedColumns[':p' . $index++]  = 'value';
        }

        $sql = sprintf(
            'INSERT INTO infiniaconfiguration (%s) VALUES (%s)',
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\InfiniaHome\DB\Configuration
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\InfiniaHome\DB\Configuration The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ConfigurationTableMap::COL_KEY)) {
            $criteria->add(ConfigurationTableMap::COL_KEY, $this->key);
        }
        if ($this->isColumnModified(ConfigurationTableMap::COL_VALUE)) {
            $criteria->add(ConfigurationTableMap::COL_VALUE, $this->value);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \InfiniaHome\DB\Configuration (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \InfiniaHome\DB\Configuration Clone of current object.
     * @throws PropelException
     */
