...
/**
 * Base class that represents a row from the 'infiniaadmins' table.
 *
 *
 *
...
abstract class Admin implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\InfiniaHome\\DB\\Map\\AdminTableMap';
    ...
    /**
     * Initializes internal state of InfiniaHome\DB\Base\Admin object.
     */
     ...
     * @return $this|\InfiniaHome\DB\Admin The current object (for fluent API support)
     */
    public function setAdminid($v)
    {
        ...
        if ($this->adminid !== $v) {
            $this->adminid = $v;
            $this->modifiedColumns[AdminTableMap::COL_ADMINID] = true;
        }
    ...
    public function setRealname($v)
    {
        ...
        if ($this->realname !== $v) {
            $this->realname = $v;
            $this->modifiedColumns[AdminTableMap::COL_REALNAME] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : AdminTableMap::translateFieldName('Adminid', TableMap::TYPE_PHPNAME, $indexType)];
            $this->adminid = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : AdminTableMap::translateFieldName('Realname', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : AdminTableMap::translateFieldName('Email', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\InfiniaHome\\DB\\Admin'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(AdminTableMap::COL_ADMINID)) {
            $modifiedColumns[':p' . $index++]  = 'adminid';
        }
        if ($this->isColumnModified(AdminTableMap::COL_REALNAME)) {
        ...
        $sql = sprintf(
            'INSERT INTO infiniaadmins (%s) VALUES (%s)',
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\InfiniaHome\DB\Admin
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\InfiniaHome\DB\Admin The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(AdminTableMap::COL_ADMINID)) {
            $criteria->add(AdminTableMap::COL_ADMINID, $this->adminid);
        }
        if ($this->isColumnModified(AdminTableMap::COL_REALNAME)) {
            $criteria->add(AdminTableMap::COL_REALNAME, $this->realname);
        }
        if ($this->isColumnModified(AdminTableMap::COL_USERNAME)) {
    ...
    /**
     * Generic method to set the primary key (adminid column).
     *
     ...
     * objects.
     *
     * @param      object $copyObj An object of \InfiniaHome\DB\Admin (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \InfiniaHome\DB\Admin Clone of current object.
     * @throws PropelException
     */
